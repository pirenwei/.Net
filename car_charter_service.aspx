<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="car_charter_service.aspx.cs" Inherits="HT.Web.car_charter_service" %>
...
<body>
    ...
    <div class="container">
        <div class="page-loct"><a href="index.aspx">首页</a> > <span>包车服务</span></div>
        <div class="news-search">
            <dl>
                <dd>
                    <label>价格：</label>
                    <select onchange="onSelclick(this)" class="dropdown">
                        <option value="car_charter_service.aspx">所有价格</option>
                        <asp:Repeater runat="server" ID="rptListPrice">
                            <ItemTemplate>
                                <option <%#Eval("title").ToString()==this.price?"selected=\"selected\"":"" %> value="car_charter_service.aspx?price=<%#Eval("title") %>&area=<%=this.area %>&areaLine=<%#this.areaLine %>"><%#Eval("title") %></option>
                            </ItemTemplate>
                        </asp:Repeater>
                    </select>
                    <em>元</em>
                ...
                <dd>
                    <label>城市：</label>
                    <select onchange="onSelclick(this)" class="dropdown">
                        <option value="car_charter_service.aspx">所有城市</option>
                        <asp:Repeater runat="server" ID="rptListArea">
                            <ItemTemplate>
                                <option <%#Eval("title").ToString()==this.area?"selected=\"selected\"":"" %> value="car_charter_service.aspx?area=<%#Eval("title") %>&price=<%=this.price %>"><%#Eval("title") %></option>
                            </ItemTemplate>
                        </asp:Repeater>
                    </select>
                    <em>市</em>
                ...
                <dd>
                    <label>路线区域：</label>
                    <select onchange="onSelclick(this)" class="dropdown">
                        <option value="car_charter_service.aspx">所有路线</option>
                        <asp:Repeater runat="server" ID="rptListAreaLine">
                            <ItemTemplate>
                                <option <%#Eval("title").ToString()==this.areaLine?"selected=\"selected\"":"" %> value="car_charter_service.aspx?area=<%=this.area %>&areaLine=<%#Eval("title") %>&price=<%=this.price %>"><%#Eval("title") %></option>
                            </ItemTemplate>
                        </asp:Repeater>
                    </select>
                </dd>
    ...
    <div class="container">
        <div class="page-tab-title">
            <ul>
                <li class="active"><a href="car_charter_service.aspx">包车服务</a></li>
                <li><a href="car_shuttle_service.aspx">接送服务</a></li>
            </ul>
        </div>
    </div>
    <div class="H10"></div>
                        ...
                        <asp:Repeater runat="server" ID="rptListHot">
                            <ItemTemplate>
                                <li>
                                    ...
                                    <p class="btn"><a href="car_charter_service_detail.aspx?cid=<%#Eval("cid") %>" class="icon-buy-btn">购买</a></p>
                                </li>
                            </ItemTemplate>
                        </asp:Repeater>
                    </ul>
    ...
    <script>
        ...
        function onBtnclick(_this) {
            var url = "car_charter_service.aspx?keywords=" + $("#txtKeywords").val();
            location.href = url;
        }
    </script>
</body>

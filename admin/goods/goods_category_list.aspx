<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="goods_category_list.aspx.cs" Inherits="HT.Web.admin.goods.goods_category_list" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="goods_category_edit.aspx?action=<%=HTEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
  ...
  <ItemTemplate>
    <tr>
      ...
      <td>
        ...
        <a href="goods_category_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a>
      </td>
      <td><asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("sort_id")%>' CssClass="sort" onkeydown="return checkNumber(event);" /></td>
      <td align="center">
        <%--<a href="goods_category_edit.aspx?action=<%#HTEnums.ActionEnum.Add %>&id=<%#Eval("id")%>">添加子类</a>--%>
        <a href="goods_category_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a>
      </td>
    </tr>
  </ItemTemplate>
  <FooterTemplate>

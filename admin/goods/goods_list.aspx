<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="goods_list.aspx.cs" Inherits="HT.Web.admin.goods.goods_list" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="goods_edit.aspx?action=<%=HTEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
          <li><asp:LinkButton ID="btnAudit" runat="server" CssClass="lock" OnClientClick="return ExePostBack('btnAudit','审核后前台将显示该信息，确定继续吗？');" onclick="btnAudit_Click"><i></i><span>审核（上架）</span></asp:LinkButton></li>
          <li><asp:LinkButton ID="btnHide" runat="server" CssClass="lock" OnClientClick="return ExePostBack('btnHide','前台将不显示该信息，确定继续吗？');" OnClick="btnHide_Click"><i></i><span>不显示（下架）</span></asp:LinkButton></li>
        ...
        <div class="menu-list">
          ...
          <div class="rule-single-select">
            <asp:DropDownList ID="ddlProperty" runat="server" AutoPostBack="True" onselectedindexchanged="ddlProperty_SelectedIndexChanged">
                ...
                <asp:ListItem Value="isHide">不显示</asp:ListItem>
                ...
                <asp:ListItem Value="isRecommend">推荐</asp:ListItem>
            </asp:DropDownList>
          </div>
        </div>
      </div>
...
<div class="table-container">
  ...
  <ItemTemplate>
    <tr>
      ...
      <td><a href="goods_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
      <td><%#new HT.BLL.goods_category().GetTitle(Convert.ToInt32(Eval("category_id")))%></td>
      <td align="center"><%#Eval("stock_quantity")%></td>
      <td><%#string.Format("{0:g}",Eval("add_time"))%></td>      
      <td align="center"><%#Eval("status").ToString()=="0"?"上架":"下架"%></td>
      <td align="center">
        <a href="goods_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a>
      </td>
    </tr>
  </ItemTemplate>
  <FooterTemplate>
  ...
  <ItemTemplate>
      <li>
        <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
          ...
          <h1><span><a href="goods_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></span></h1>
          <div class="remark">
            <%#Eval("zhaiyao").ToString() == "" ? "暂无内容摘要说明..." : Eval("zhaiyao").ToString()%>
          </div>
          <div class="foot">
            <p class="time"><%#string.Format("{0:yyyy-MM-dd HH:mm:ss}", Eval("add_time"))%>
                &nbsp; <%#Eval("status").ToString()=="0"?"上架":"下架"%>
            </p>
            <a href="goods_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
          </div>
        </div>
      </li>
  </ItemTemplate>

...
namespace HT.Web.admin
{
    public partial class login : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                txtUserName.Text = Utils.GetCookie("HTRememberName");
            }
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                new BLL.manager_log().Add(model.id, model.user_name, HTEnums.ActionEnum.Login.ToString(), "用户登录");
            }
            //写入Cookies
            Utils.WriteCookie("HTRememberName", model.user_name, 14400);
            Utils.WriteCookie("AdminName", "HT", model.user_name);
            Utils.WriteCookie("AdminPwd", "HT", model.password);

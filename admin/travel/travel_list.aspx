<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="travel_list.aspx.cs" Inherits="HT.Web.admin.travel.travel_list" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="travel_edit.aspx?action=<%=HTEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
...
<div class="table-container">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
    <tr>
      ...
      <th align="left">标题</th>
      <th width="10%">发布者</th>
      <th align="left" width="16%">行程日期</th>
      <th width="10%">是否公开</th>
      <th width="10%">状态</th>
      <th width="10%">操作</th>
    </tr>
  ...
  <ItemTemplate>
    <tr>
      ...
      <td><a href="travel_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
      <td align="center"><%#Eval("user_name")%></td>
      <td><%#string.Format("{0:g}",Eval("begin_date"))%>~<%#string.Format("{0:g}",Eval("end_date"))%></td>
      <td align="center"><%#Eval("is_open").ToString()=="1"?"公开":"隐藏"%></td>
      <td align="center"><%#Eval("status").ToString()=="0"?"正常":"待审核"%></td>
      <td align="center">
        <a href="travel_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a>
         <%#Utils.ObjToInt(Eval("group_id"))>0?"<a href=\"travel_group_edit.aspx?action=Edit&id="+Eval("group_id")+"\">查看组团</a>":""%>
      </td>
    </tr>
  </ItemTemplate>
  <FooterTemplate>
  ...
  <ItemTemplate>
      <li>
        <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
          ...
          <h1><span><a href="travel_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></span></h1>
          <div class="remark">
            <%--<%#Eval("remark").ToString() == "" ? "暂无说明..." : Eval("remark").ToString()%>--%>
            <%#Utils.ObjToInt(Eval("group_id"))>0?"<a href=\"travel_group_edit.aspx?action=Edit&id="+Eval("group_id")+"\"><b>查看组团</b></a>":""%>
          </div>
          <div class="foot">
            <p class="time"><%#string.Format("{0:yyyy-MM-dd HH:mm:ss}", Eval("add_time"))%>
                &nbsp; <%#Eval("status").ToString()=="0"?"正常":"待审核"%>
            </p>
            <a href="travel_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
          </div>
        </div>
      </li>
  </ItemTemplate>

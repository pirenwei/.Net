...
namespace HT.Web.admin.users
{
    public partial class oauth_app_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_oauth", HTEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("oauth_app_list_page_size", "HTPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("oauth_app_list_page_size", "HTPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_oauth", HTEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.user_oauth_app bll = new BLL.user_oauth_app();
            ...
            AddAdminLog(HTEnums.ActionEnum.Edit.ToString(), "保存OAuth配置排序"); //记录日志
        ...
        {
            ChkAdminLevel("user_oauth", HTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_oauth_app bll = new BLL.user_oauth_app();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            AddAdminLog(HTEnums.ActionEnum.Delete.ToString(), "删除OAuth配置成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

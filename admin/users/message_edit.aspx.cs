...
namespace HT.Web.admin.users
{
    public partial class message_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == HTEnums.ActionEnum.View.ToString())
            {
                this.action = HTEnums.ActionEnum.View.ToString();//修改类型
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_message", HTEnums.ActionEnum.View.ToString()); //检查权限
                if (action == HTEnums.ActionEnum.View.ToString()) //修改
                {
                    ShowInfo(this.id);
                }
                ...
                if (!string.IsNullOrEmpty(userids))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            if (item["user_name"].ToString() != "")
                            {
                                this.usernames += item["user_name"] + ",";
                            }
                        }
                    }
                    txtUserName.Text = this.usernames;
                }
        ...
        {
            ChkAdminLevel("user_message", HTEnums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
                JscriptMsg("发送过程中发生错误！", "");
                return;
            }
            AddAdminLog(HTEnums.ActionEnum.Add.ToString(), "发送站内短消息"); //记录日志

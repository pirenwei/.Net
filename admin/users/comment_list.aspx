<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="comment_list.aspx.cs" Inherits="HT.Web.admin.users.comment_list" %>
...
<div class="table-container">
  ...
  <ItemTemplate>
    <tr>
      <td class="comment">
        <div class="title">
          <span class="note"><i><%#Eval("user_name")%></i><i><%#Eval("add_time")%></i><i class="reply"><a href="comment_edit.aspx?id=<%#Eval("id")%>">编辑</a></i></span>
          <b><asp:CheckBox ID="chkId" CssClass="checkall" runat="server" /><asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" /></b>
          【<%#HT.Web.UI.WebUI.GetDataType(Utils.ObjToInt(Eval("fk_id")),Utils.ObjToInt(Eval("fk_type")))%>】
          <%#HT.Web.UI.WebUI.GetDataTitle(Utils.ObjToInt(Eval("fk_id")),Utils.ObjToInt(Eval("fk_type")))%>
        </div>
        <div class="ask">
          <%#Convert.ToInt32(Eval("is_lock")) == 1 ? "<b style=\"color:red\">【待审核】</b>" : ""%><%#Eval("content")%>
          <asp:Repeater runat="server" ID="rptListRp">
              <ItemTemplate>
                 <div class="answer">
                    <b><%#Eval("user_name")%>回复：</b><%#Eval("content")%>
                    <span class="time"><%#Eval("add_time")%>
                        &nbsp;&nbsp;<a href="comment_edit.aspx?id=<%#Eval("id")%>">编辑</a>
                    </span>
                 </div>
              </ItemTemplate>
          </asp:Repeater>

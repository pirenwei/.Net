...
namespace HT.Web.admin.users
{
    public partial class sms_template_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(HTEnums.ActionEnum.Add.ToString(), "添加短信模板:" + model.title); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(HTEnums.ActionEnum.Edit.ToString(), "修改短信模板:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == HTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("user_sms_template", HTEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("user_sms_template", HTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", string.Empty);
                    return;

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="exchange_gift_list.aspx.cs" Inherits="HT.Web.admin.gb.exchange_gift_list" %>
...
<div id="floatHead" class="toolbar-wrap">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="exchange_gift_edit.aspx?action=<%=HTEnums.ActionEnum.Add %>"><i></i><span>新增</span></a></li>
...
<div class="table-container">
  ...
  <ItemTemplate>
    <tr>
      ...
      <td><a href="exchange_gift_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
      <%--<td><%#Eval("begin_time")%>——<%#Eval("end_time")%></td>--%>
      <td align="center"><%#Eval("point")%></td>
      <td><%#string.Format("{0:g}",Eval("add_time"))%></td>      
      <td align="center"><%#Eval("status").ToString()=="0"?"正常":"无效"%></td>
      <td align="center">
        <a href="exchange_gift_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a>
      </td>
    </tr>
  </ItemTemplate>
  <FooterTemplate>
  ...
  <ItemTemplate>
      <li>
        <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
          ...
          <h1><span><a href="exchange_gift_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></span></h1>
<%--          <div class="remark">
            <%#Eval("begin_time")%>——<%#Eval("end_time")%>
          </div>--%>
          <div class="foot">
            <p class="time"><%#string.Format("{0:yyyy-MM-dd HH:mm:ss}", Eval("add_time"))%>
                &nbsp; <%#Eval("status").ToString()=="0"?"正常":"无效"%>
            </p>
            <a href="exchange_gift_edit.aspx?action=<%#HTEnums.ActionEnum.Edit %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
          </div>
        </div>
      </li>
  </ItemTemplate>

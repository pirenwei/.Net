        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (admin_info != null)
                {
                    ...
                    Model.manager_log model1 = bll.GetModel(admin_info.user_name, 1, HTEnums.ActionEnum.Login.ToString());
                    if (model1 != null)
                    {
                        //本次登录
                        litIP.Text = model1.user_ip;
                    }
                    Model.manager_log model2 = bll.GetModel(admin_info.user_name, 2, HTEnums.ActionEnum.Login.ToString());
                    if (model2 != null)
                    {
                        //上一次登录
